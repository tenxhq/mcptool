...
impl ArgumentParser {
    ...
    /// Parse arguments from key=value format into a HashMap<String, String>
    /// Used for prompts which take string-only arguments
    pub fn parse_key_value_strings(args: Vec<String>) -> Result<Option<HashMap<String, String>>> {
        if args.is_empty() {
            return Ok(None);
        }
